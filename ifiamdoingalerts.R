...
load_n_mask <- function(selected_tile, aoi, year, dsn = NA){
  if (!is.na(dsn)) {
    print(paste0(" - Reading tile from disk and masking at ", format(Sys.time(), "%a %b %d %X %Y")))    
    raster_glad <- mask(rast(paste0(
      dsn, "/alertDate", substr(year, 3, 4), "_", selected_tile, ".tif"
    )), aoi) %>% mask(., rast(paste0(
      dsn, "/alert", substr(year, 3, 4), "_", selected_tile, ".tif"
    )), maskvalues = 2)
    raster_glad[raster_glad == 0] <- NA
  } else {
    print(paste0(" - Downloading and masking tile at ", format(Sys.time(), "%a %b %d %X %Y")))    
    raster_glad <- mask(rast(paste0(
      ...
      "https://storage.googleapis.com/earthenginepartners-hansen/GLADalert/C2/current/alert",
      substr(year, 3, 4), "_", selected_tile, ".tif"
    )), maskvalues = 2)
    raster_glad[raster_glad == 0] <- NA
  }
...
get_first_last_date <- function(glad_raster, deforestation, clumped, year) {
  first_last_dates <- data.frame(first = zonal(glad_raster, deforestation, "min",   na.rm = TRUE),
...
isdo <- function(country, year, pixels = 16, dsn = NA) {
  ...
  if (is.character(tiles)) return(tiles)
  ...
  for (i in 1:length_tiles) {
    ...
    if (is.character(clumped_sieved)) {next
    } else {
  ...
  treated_at_boundries <- st_join(st_as_sf(treated_at_boundries), st_as_sf(treat_at_boundries)) %>%
    ...
    mutate(persist_days = as.numeric(trunc(difftime(
      as.Date(last), as.Date(first), units = "day")) + 1))
